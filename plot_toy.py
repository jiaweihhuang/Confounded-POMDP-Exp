...
def main():
    ...
    log_path = './log/Toy_Linear_Est_DR/FDim{}_ObsNoise{}_InNoise{}_Alpha{}_S{}.pickle'.format(args.feature_dim, obs_noise, args.inherent_noise, args.alpha, sample_size)
    ...
    # use w as x_coord
    x_coord = list(AllEstResults.keys())
    x_coord.sort()
    ...
    for w in x_coord:
        ...
        Baseline_Bias[w] = np.abs(np.mean(AllEstResults[w]['Baseline'] / True_Value - 1.0))
        PO_Bias[w] = np.abs(np.mean(AllEstResults[w]['PO_Est'] / True_Value - 1.0))
    ...
    if args.plot_bias:
        Baseline_log_Bias = np.log([Baseline_Bias[w] for w in Baseline_Bias.keys()])
        PO_log_Bias = np.log([PO_Bias[w] for w in PO_Bias.keys()])

        print(Baseline_log_Bias)
        print(PO_log_Bias)

        Baseline_ErrBar = np.array([Baseline_Bias_ErrBar[w] for w in Baseline_Bias_ErrBar.keys()]).transpose()
        PO_ErrBar = np.array([PO_Bias_ErrBar[w] for w in PO_Bias_ErrBar.keys()]).transpose()

        plt.errorbar(range(len(x_coord)), Baseline_log_Bias, yerr=Baseline_ErrBar, color='b', label='Baseline', marker='o', capsize=capsize, linewidth=linewidth, markersize=10)
        plt.errorbar(range(len(x_coord)), PO_log_Bias, yerr=PO_ErrBar, color='r', label='Ours', marker='s', capsize=capsize, linewidth=linewidth, markersize=10)
        plt.ylabel('Log of Bias (relative)', fontsize=12)
    else:
        Baseline_log_MSE = np.log([Baseline_MSE[w] for w in Baseline_MSE.keys()])
        PO_log_MSE = np.log([PO_MSE[w] for w in PO_MSE.keys()])

        print(Baseline_log_MSE)
        print(PO_log_MSE)
        
        Baseline_ErrBar = np.array([Baseline_MSE_ErrBar[w] for w in Baseline_MSE_ErrBar.keys()]).transpose()
        PO_ErrBar = np.array([PO_MSE_ErrBar[w] for w in PO_MSE_ErrBar.keys()]).transpose()

        plt.errorbar(range(len(x_coord)), Baseline_log_MSE, yerr=Baseline_ErrBar, color='b', label='Baseline', marker='o', capsize=capsize, linewidth=linewidth, markersize=10)
        plt.errorbar(range(len(x_coord)), PO_log_MSE, yerr=PO_ErrBar, color='r', label='Ours', marker='s', capsize=capsize, linewidth=linewidth, markersize=10)
        plt.ylabel('Log MSE (relative)', fontsize=fontsize)

    title = r'1-D Process (Obs Noise = $\mathcal{N}$(0,' + '{}))'.format(args.obs_noise)

    plt.legend(fontsize=fontsize, loc='best')
    plt.title(title, fontsize=fontsize+3)
    plt.xticks(range(len(x_coord)), x_coord, fontsize=fontsize)
    plt.yticks(fontsize=fontsize)
    plt.xlabel('Choice of w', fontsize=fontsize)
    
    if not os.path.exists('ExpFigures'):
        os.makedirs('ExpFigures')

    suffix = 'Bias' if args.plot_bias else 'MSE'
    plt.savefig('ExpFigures/Toy_N{}_{}.png'.format(args.obs_noise, suffix))
    plt.show()
